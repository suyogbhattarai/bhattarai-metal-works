...
export default function ProductForm({ initialData, onSubmit, submitting, categories, onCancel }: ProductFormProps) {
    ...
    useEffect(() => {
        ...
        if (initialData) {
            setBasicInfo({
                ...
                category: typeof initialData.category === 'object' ? initialData.category?.id?.toString() || '' : initialData.category?.toString() || '',
                ...
                customization_note: initialData.customization_note || ''
            });
            ...
            if (initialData.materials) {
                const matIds = initialData.materials.map((m: any) => typeof m === 'object' ? m.id : m);
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            const newImgObjs = files.map(file => ({
                ...
                isPrimary: newImages.length === 0 && existingImages.length === 0
            }));
    ...
    return (
        <div className="flex flex-col h-full bg-[#111e48] rounded-2xl shadow-xl overflow-hidden border border-white/5">
            {/* Tabs */}
            <div className="flex border-b border-white/10 bg-[#0a1642] overflow-x-auto shrink-0 no-scrollbar">
                {tabs.map(tab => (
                    <button
                        key={tab.id}
            ...
            {/* Content */}
            <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar bg-[#0e1736]">
